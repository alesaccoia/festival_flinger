...
;;;  ABC diphones: male Mexican Spanish collected June 1997 by OGI/UDLA   ;;
...
(defvar abcdi_grouped_or_ungrouped 'grouped)

;; Location at runtime
(defvar ogi_abc_diphone_dir (cdr (assoc 'ogi_abc_diphone voice-locations))
  "ogi_abc_diphone_dir
  The default directory for the abc diphone database.")
(set! load-path (cons (path-append ogi_abc_diphone_dir "festvox") load-path))

(define (voice_ogi_abc_diphone)
"(voice_ogi_abc_diphone)
 Set up the current voice to be a Mexican Spanish male ABC using
 the ABC diphone set and OGIresLPC."
...
(set! abcdi_smooth_spectra   ;;; max number of frames to use in smoothing LPC at joins
      '(spectra_smooth
        ((vc + 10)   ; vowel
         (ctype s 0) ; stop
         (ctype f 2) ; fricative
...
(set! abcdi_smooth_power   ;;; max number of frames to use in smoothing power at joins
      '(power_smooth
        ((vc + 10)   ; vowel
         (ctype s 0) ; stop
         (ctype f 2) ; fricative
...
(set! abcdi_OGI_syn_params  
 (list
  ...
  abcdi_smooth_spectra
  abcdi_smooth_power
  ))


(define (init_ogi_abc_diphone)
"(init_ogi_abc_diphone)
  Initialise the ABC diphone database."
  (set! voicename 'ogi_abc_diphone)
(if (equal? abcdi_grouped_or_ungrouped 'ungrouped)
    ;;  ungrouped
    (OGIdbase.init
     (list
      '(dbname ogi_abc_diphone)
      (list 'unitdic_file (path-append ogi_abc_diphone_dir "festvox" "abcdiph.msec"))
      '(phoneset "span_mx")  
      '(base_dir "/u/macon/TTS/tts_data/abc/")
      '(lpc_dir "lpc16/")
      '(lpc_ext ".lpc")
      '(exc_dir "lpc16/")
      '(exc_ext ".res")
    ...
    (OGIdbase.init   
     (list
      '(dbname ogi_abc_diphone)
      (list 'groupfile (path-append ogi_abc_diphone_dir "group" (string-append voicename "_reslpc.group")))
      '(data_type "resLPC")
      '(access_mode "ondemand")
      ))
    t)
...
(proclaim_voice
 ...
 '((language spanish)
   ...
   (dialect mexican)
   (description
    "This voice provides a Mexican Spanish male voice using a
